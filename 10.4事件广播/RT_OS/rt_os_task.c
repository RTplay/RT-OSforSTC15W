...
// 返回：OS_ERR_NONE:是成功
// OS_ERR_TASK_ID_INVALID：如果试图改变idle任务的优先级或者任务不存在或者任务ID超过最大任务数量
// OS_ERR_PRIO_INVALID: 如果试图更改为idle任务的优先级（0）是不允许的
// OS_ERR_PRIO_ERR: 其他错误
...
void idle_task_0(void)
{
    ...
    while (1) {
        ...
        if (i != TASK_SIZE) {//队列中有就绪任务，进行任务切换
            os_tcb[OS_IDLE_TASKID].OSTCBStatus = OS_STAT_RDY;
            OS_EXIT_CRITICAL();
            OS_TASK_SW();
        }
