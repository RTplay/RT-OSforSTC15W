...
static void os_flag_all_task_rdy(u8 flag_index);
...
OS_FLAGS os_flag_post (u8 flag_index, OS_FLAGS flags, u8 opt, u8 *err)
{
    ...
    if ((flag[flag_index].OSFlagWaitType & OS_FLAG_CONSUME) != 0)
        consume = TRUE;
    else
        consume = FALSE;
        ...
        case OS_FLAG_WAIT_SET_ALL:                          /* See if all req. flags are set for current node */
            ...
            if (flags_rdy == flag[flag_index].OSFlagInterestedFlags) {
                if (consume)
                    ...
                    os_flag_all_task_rdy(flag_index);       /* Make all task to RDY                           */
